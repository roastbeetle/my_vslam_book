...
int main ( int argc, char** argv )
{
    ...
    for(int i=0; i<2; i++){
        if(i==0)
            mpts.push_back(thread(dovo, std::ref(mWork), vo, camera, rgb_files, depth_files, rgb_times, depth_times));
        if(i==1)
            mpts.push_back(thread(viewer, std::ref(mView), vo, camera, rgb_files, depth_files, rgb_times, depth_times));
    }  
    for(int i=0; i<2; i++){
       mpts[i].join();
    }  
...
void dovo(std::mutex& m, myslam::VisualOdometry::Ptr vo, myslam::Camera::Ptr camera, 
            vector<string> &rgb_files, vector<string> &depth_files, vector<double> &rgb_times,  vector<double> &depth_times){
...
void viewer(std::mutex& m, myslam::VisualOdometry::Ptr vo, myslam::Camera::Ptr camera, 
            vector<string> &rgb_files, vector<string> &depth_files, vector<double> &rgb_times,  vector<double> &depth_times){
    // visualization
    std::unique_lock<mutex> lock(m);
